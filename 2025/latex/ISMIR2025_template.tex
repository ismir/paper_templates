...
\textbf{To enhance accessibility, we encourage the authors to provide alt text to the figures.} For \LaTeX\ users, this can be done by providing the `alt' option when calling \texttt{\textbackslash includegraphics}, for example, \texttt{\textbackslash includegraphics[alt=\{example alt text\}]\{example.png\}}. For Word users, this can be done by right-clicking the image and selecting ``Edit Alt Text,'' or alternatively, by selecting ``Picture Format > Alt Text.'' \textbf{To enhance colorblind-friendliness, we encourage the authors to adopt a color blind friendly color palette when making plots.} For Matplotlib users, the `tableau-colorblind10' and `petroff10' color palettes would be good options, which can be enabled by \texttt{plt.style.use('tableau-colorblind10')} and \texttt{plt.style.use('petroff10')}.
...
\textbf{The camera-ready version should include the names, affiliations and email addresses of the authors.} Authors' names are centered. The lead author's name is to be listed first (left-most), and the co-authors' names after. If the addresses for all authors are the same, include the address only once, centered. If the authors have different addresses, put the addresses, evenly spaced, under each authors' name. To display the author information in \LaTeX, please remove the global option `submission' when importing the ismir package (i.e., \texttt{\textbackslash usepackage\{ismir\}} in line 16). \textbf{Please make sure that the author names and paper title are shown correctly in the copyright notice when submitting the camera-ready version}.
