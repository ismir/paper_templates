...
\textbf{You will also need to insert the appropriate author names and paper title in the copyright notice when submitting the camera-ready version}. For \LaTeX\ users, this will be handled by the template automatically. For Word users, this has to be done manually.

\textbf{You must also remove all line numbers from the final camera-ready version.} This can be done in \LaTeX\ by commenting out the command \verb|\linenumbers| on Line 21. This can be done in Microsoft Word by selecting ``Layout > Line Numbers > None.''
